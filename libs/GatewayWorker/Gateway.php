...
class Gateway extends Worker
{
    ...
    {
        if (!$this->getStore('gateway')->set('client_id-' . $global_client_id, $address)) {
            $msg = 'storeClientAddress fail.';
            if (get_class($this->getStore('gateway')) == 'Memcached') {
                $msg .= " reason :" . $this->getStore('gateway')->getResultMessage();
            }
            $this->log($msg);
            return false;
        }
    ...
    {
        $this->getStore('gateway')->delete('client_id-' . $global_client_id);
    }
