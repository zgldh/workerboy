...
class BusinessWorker extends Worker
{
    ...
    protected function onWorkerStart()
    {
        ...
        \GatewayWorker\Lib\Gateway::$store_config = $this->getStoreConfig();
    ...
    public function onGatewayMessage($connection, $data)
    {
        ...
        $cmd = $data['cmd'];
        // 尝试执行 $this->event->onConnection、$this->event->onMessage、$this->event->onClose
        try {
            switch ($cmd) {
                case GatewayProtocol::CMD_ON_CONNECTION:
                    $this->event->onConnect(Context::$client_id);
                    break;
                case GatewayProtocol::CMD_ON_MESSAGE:
                    $this->event->onMessage(Context::$client_id, $data['body']);
                    break;
                case GatewayProtocol::CMD_ON_CLOSE:
                    $this->event->onClose(Context::$client_id);
                    break;
            }
        } catch (\Exception $e) {
    ...
    public function checkGatewayConnections()
    {
        ...
        $addresses_list = $this->getStore('gateway')->get($key);
    ...
    public function tryToDeleteGatewayAddress($addr, $errstr)
    {
        ...
        if ($this->_badGatewayAddress[$addr]++ > self::MAX_RETRY_COUNT) {
            ...
            $this->getStore('gateway')->set($key, $addresses_list);
            Lock::release();
